...
public class AuthRepository(AuthContext context) : IAuthRepository
{
    private readonly AuthContext _context = context;

    public async Task<APIResponse> Add(Auth auth)
    {
        try
        {
    ...
    public async Task<APIResponse> Update(Auth auth)
    {
        try
        {
